...
```{r, V, echo=FALSE}
V = 20 # TODO: More?
pgos <- seq(0.5, 0.95, length.out=19)
...
f <- function (x) filter(x, minima & prize_phase != 'P1')
g <- function (x) filter(x, minima)
tmp <- resampled %>%
  # Only compare cost diff in projs with go-decison after intervention
  filter(int_priv_enpv >= 0) %>%
  #filter(pred_go <= 0.9 & pred_go >= 0.4) %>%
  mutate(diff = inef_dir_enpv - ind_enpv) %>%
  ...
  ungroup
tmp %>%
  ggplot(aes(publ_inefficiency, pred_go, z=mean_diff)) +
  facet_grid(src ~ prize_phase) +
  geom_tile(aes(fill=mean_diff)) +
  geom_point(data = g, aes(publ_inefficiency, pred_go), shape='x') +
  ...
  ylab('P(go)') +
  xlab('Inefficiency') +
  scale_fill_gradientn(colors = c('darkred', 'red', 'white', 'blue', 'black'),
                       limits = c(min(tmp$mean_diff), max(tmp$mean_diff)),
                       values = rescale(c(min(tmp$mean_diff), 0, max(tmp$mean_diff))),
                       name = 'Cost savings of direct funding per go-decision (million USD)',
                       guide = guide_colorbar(barwidth=40, title.position='top'),
...
tmp %>%
  ...
  scale_fill_gradientn(colors = c('darkred', 'red', 'white', 'blue', 'black'),
                       limits = c(min(tmp$mean_diff), max(tmp$mean_diff)),
                       values = rescale(c(min(tmp$mean_diff), 0, max(tmp$mean_diff))),
                       name = 'Cost savings of direct funding per market entry (million USD)',
                       guide = guide_colorbar(barwidth=40, title.position='top'),
