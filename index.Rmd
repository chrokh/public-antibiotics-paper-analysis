...
tmp <- resampled %>%
  ...
  group_by(src, prize_phase, publ_inefficiency) %>%
  mutate(minima = near(abs(mean_diff), min(abs(mean_diff))) & abs(mean_diff) <= sensitivity) %>%
  ungroup
...
tmp %>%
  ...
  #geom_line(data=f, aes(publ_inefficiency, pred_go)) +
  #geom_point(data = g, aes(publ_inefficiency, pred_go), shape=1) +
  geom_smooth(data = f, aes(publ_inefficiency, pred_go),
              #method='loess',
              method=lm, formula=y~bs(x, 3),
              se=FALSE, color='black', size=0.5) +
...
Minima above must be within range [`r -sensitivity`, `r sensitivity`].
