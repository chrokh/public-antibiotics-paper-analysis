...
Let us first add public discount rates like this:

```{r}
rates = runif(N, min=public_discount_rate_min, max=public_discount_rate_max)
comparable_years <- comparable_years %>%
  # Private discount rate
  rename(discount_rate_priv = discount_rate) %>%
  # Public discount rate (keep consistent within subjects)
  group_by(subject) %>%
  mutate(discount_rate_publ = rates[subject]) %>%
  ungroup
```

and then compute private, direct, and indirect ENPV like this:
...
Finally, we can extract the different ENPV values from the perspective of pre-clinical, like this:
