...
phasely_valuation_long <- phasely_valuation %>%
  select(src, intervention, subject, from, phase, tot_prizes,
         cashflow, ev, pv, epv,
         cum, env, npv, enpv,
         enpv_dir, enpv_ind) %>%
  gather('metric', 'value', -src, -intervention, -subject, -from, -phase, -tot_prizes) %>%
  transform(metric = factor(metric, levels = c(metrics, cum_metrics, add_metrics)))

# Final valuations
phasely_final_valuation <- phasely_valuation %>%
  group_by(src, intervention, subject, from, tot_prizes) %>%
  arrange(phase) %>%
...
phasely_final_valuation_long <- phasely_valuation_long %>%
  ...
  group_by(src, intervention, subject, from, metric, tot_prizes) %>%
  arrange(phase) %>%
...
yearly_valuation_long <- yearly_valuation %>%
  select(src, intervention, subject, from, year, tot_prizes,
         cashflow, ev, pv, epv,
         cum, env, npv, enpv,
         enpv_dir, enpv_ind) %>%
  gather('metric', 'value', -src, -intervention, -subject, -from, -year, -tot_prizes) %>%
  transform(metric = factor(metric, levels = c(metrics, cum_metrics, add_metrics)))
...
yearly_final_valuation <- yearly_valuation %>%
  ...
  summarize(cum = tail(cum, n=1),
            ...
            enpv_dir = tail(enpv_dir, n=1),
            enpv_ind = tail(enpv_ind, n=1),
            time_to = tail(time_to, n=1),
            prob_to = tail(prob_to, n=1)) %>%
  ungroup
...
finals <- yearly_final_valuation %>%
  ...
  filter(from == 'PC') %>%
  # Only keep identifying and valuation columns (+ prob_to, time_to)
  select(src, intervention, subject, intervention, tot_prizes,
         cum, env, npv, enpv, enpv_ind, enpv_dir, prob_to, time_to) %>%
  ...
  merge(., ., by=c('src', 'subject', 'prob_to', 'time_to')) %>%
...
print(finals %>%
  ...
  ggplot(aes(tot_prizes.y, enpv.y - enpv.x, col=intervention.y)) +
...
print(finals %>%
  filter(intervention.x == 'NONE') %>%
  ggplot(aes(tot_prizes.y, enpv.y - enpv.x, color=intervention.y)) +
  geom_smooth(method='lm', se=FALSE) +
  geom_point(shape=1) +
  xlab('Prize') +
...
finals %>%
  filter(intervention.x == 'NONE') %>%
  ggplot(aes(tot_prizes.y, enpv.y >= 0, color=intervention.y)) +
  geom_jitter(shape=1) +
...
go_model <- glm(
  (enpv.y >= 0) ~ tot_prizes.y * interaction(src, intervention.y),
  data = nos,
...
nos %>%
  ggplot(aes(tot_prizes.y, pred.go.y*100, color=intervention.y)) +
  geom_line() +
  scale_x_continuous(trans = 'log',
                     breaks = c(1 %o% 10^(0:10)),
                     labels = trans_format('log10', math_format(10^.x))) +
...
```{r, predict-go}
mod <- glm(
  (enpv.y >= 0) ~ tot_prizes.y * interaction(src, intervention.x, intervention.y),
  data = finals,
  family = binomial(link='logit'))
...
finals %>%
  ...
  filter(intervention.x == 'NONE') %>%
  ggplot(aes(tot_prizes.y, -enpv_ind.y, color=intervention.y)) +
  geom_point(shape=1) +
  geom_smooth(method='lm', se=FALSE) +
...
finals %>%
  filter(intervention.x == 'NONE' & enpv.y >= 0) %>%
  mutate(enpv_ind_per_exit.y = enpv_ind.y/prob_to) %>%
  ggplot(aes(tot_prizes.y, -enpv_ind_per_exit.y, color=intervention.y)) +
  geom_smooth(method='lm', se=FALSE) +
...
finals %>%
  filter(intervention.x == 'NONE' &
         pred.go.y < go_lim_up &
         enpv.y >= 0) %>%
  ggplot(aes(pred.go.y*100, -enpv_ind.y/prob_to, color=intervention.y)) +
  geom_smooth(method='loess', se=FALSE) +
  facet_wrap(. ~ src, scales='free', nrow=2) +
  xlab('Probability of go-decision (%) under prize size') +
