...
# Input data
We begin by loading external libraries, setting basic document options, seeding the randomizer and deciding on an `N` which will dictate the number of project samples that we'll draw from each dataset.
...
Sertkaya et. al differentiates between different types of antibiotics on the basis of target indication.
In this analysis we consider the data for each target indication a separate dataset.


### Distribution

```{r, dist-sertkaya, results='asis'}
...
kable(sertkaya2014_phase_dist)
kable(sertkaya2014_market_size_dist)
kable(sertkaya2014_market_share_dist)
```

The time, cost, and probability distributions are visualized in the plots below, in respective order.

```{r, plot-sertkaya-dist, echo=FALSE}
pos = position_dodge(0.9)
sertkaya2014_phase_dist %>%
  ggplot(aes(indication, color=indication)) +
  geom_point(aes(y=time_min), position=pos) +
  geom_point(aes(y=time_max), position=pos) +
  geom_linerange(aes(ymin=time_min, ymax=time_max), position=pos) +
...
sertkaya2014_phase_dist %>%
  ggplot(aes(indication, color=indication)) +
  geom_pointrange(aes(y=cost_mid, ymin=cost_min, ymax=cost_max), position=pos) +
  facet_wrap(. ~ phase, scale='free') +
...
sertkaya2014_phase_dist %>%
  ggplot(aes(indication, color=indication)) +
  geom_pointrange(aes(y=prob_mid, ymin=prob_min, ymax=prob_max), position=pos) +
  facet_wrap(. ~ phase, scale='free') +
...
### Sampling
We now sample the Sertkaya dataset and add discount rate samples.

```{r, sample-sertkaya}

# Sample development phases
dist <- sertkaya2014_phase_dist %>% arrange(indication, phase) # alias
variations <- length(unique(dist$phase)) * length(unique(dist$indication))
sertkaya2014_dev <-
  data.frame(# TODO: Does it really make sense that I allow 'recycling' of
             # subject IDs across datasets/srcs?
             subject = rep(1:N, each = variations),
             src = rep(unique(dist$indication), each = length(unique(dist$phase))),
             phase = dist$phase,
             time = runif(N * variations, dist$time_min, dist$time_max),
             cost = rtriangle2(N * variations, dist$cost_min, dist$cost_max, dist$cost_mid),
             prob = rtriangle2(N * variations, dist$prob_min, dist$prob_max, dist$prob_mid),
             sales = 0)

## Sample market
years       <- sertkaya2014_market_share_dist$year
indications <- sertkaya2014_market_size_dist$indication
sertkaya2014_market <-
  data.frame(subject = rep(1:N, each=length(years)*length(indications)),
             src = rep(indications, each=length(years)),
             phase = paste('M', rep(years, length(indications)), sep=''), # TODO: Use MARKET_PHASES vector
             time = 12, # months
             cost = 0,
             prob = 100, # percentage
             share = runif(N * length(indications),
                           sertkaya2014_market_share_dist$min,
                           sertkaya2014_market_share_dist$max),
             size = rep(runif(N,
                          sertkaya2014_market_size_dist$min,
                          sertkaya2014_market_size_dist$max),
                        each = length(unique(years)))
             ) %>% mutate(sales = share * (size / 100))

kable(head(sertkaya2014_market))

# Join development and market data
sertkaya2014 <-
  rbind(sertkaya2014_dev,
        select(sertkaya2014_market, -c(share, size))
        ) %>%
...
# Sample discount rates from the same distribution across all phases and indications.
discount_rates <- runif(N * length(unique(sertkaya2014$src)), 0.09, 0.24)
sertkaya2014$discount_rate_priv <-
  rep(discount_rates, each=length(unique(sertkaya2014$phase)))
```

<div class="alert alert-danger">
NOTE1: AFTER SAMPLING THIS WE MUST ALSO SAMPLE THE ACTIVITIES FROM TABLE 9
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
NOTE2: WE'RE NOT ACTUALLY TAKING THE "PRODUCT LAUNCH SUCCESS PROBABILITY" INTO ACCOUNT (see Table 12)
</div>
...
TODO: The time, cost, and probability distributions are visualized in the plots below, in respective order.

```{r, plot-driveab-dist, echo=FALSE}
#pos = position_dodge(0.9)
#time %>%
#  ggplot(aes(phase, color=phase)) +
#  geom_point(aes(y=min), position=pos) +
#  geom_point(aes(y=max), position=pos) +
#  geom_linerange(aes(ymin=min, ymax=max), position=pos) +
#  facet_wrap(. ~ phase, scale='free') +
#  ylab('Time (months)') +
#  xlab(element_blank()) +
#  theme(axis.text.x = element_text(angle=90, hjust=1, vjust=1),
...
#cost %>%
#  ggplot(aes(phase, color=phase)) +
#  geom_pointrange(aes(y=mid, ymin=min, ymax=max), position=pos) +
#  facet_wrap(. ~ phase, scale='free') +
#  ylab('Cost (million USD)') +
#  xlab(element_blank()) +
#  theme(axis.text.x = element_text(angle=90, hjust=1, vjust=1),
...
```{r, sample-driveab}
# Sample development phases
variations <- length(unique(time$phase))
driveab2018_dev <- data.frame(
  subject = rep(1:N, each = variations),
  phase = DEVELOPMENT_PHASES,
  time = rtriangle2(N * variations, time$min, time$max, time$mid),
  cost = rtriangle2(N * variations, cost$min, cost$max, cost$mid),
  prob = rtriangle2(N * variations, prob$min, prob$max, prob$mid),
  sales = 0
  )

# Sample market data
variations <- length(MARKET_PHASES)
driveab2018_sales <-
  data.frame(subject = rep(1:N, each = variations),
             phase = MARKET_PHASES,
             time = 12, # months
             cost = 0, # million usd
             prob = 100, # percentage
             market_years = params$market_years,
             tot_sales = rep(rtriangle(N,
                                   params$tot_sales_min,
                                   params$tot_sales_max,
                                   params$tot_sales_mid),
                             each = variations)
             # TODO: The slope does add up to the full market size, but are we
             # sloping in the same way as in the DRIVE-AB report? Or did we start
             # from 0 in the report?
             ) %>%
             mutate(slope = (tot_sales * 2 / (market_years + 1)) / market_years,
                          sales = 1:variations * slope) %>%
             select(-market_years, -tot_sales, -slope) # Remove temp columns
...
# Sample discount rates from the same distribution across all phases
discount_rates <- runif(N, 0.05, 0.30)
driveab2018$discount_rate_priv <-
  rep(discount_rates, each = length(unique(driveab2018$phase)))
```
...
# TODO: When I add parameters (before this point) to the dataset that are
# dependent on order I should explicitly reorder the dataset like I do here:
phases <- phases %>% arrange(phase, src, subject, phase)
# TODO: Should the same subject have the same public discount rate across data sets?
phases$discount_rate_publ <- runif(N * length(sources), public_discount_rate_min, public_discount_rate_max)
# TODO: Should the same subject have the same inefficiency across data sets?
phases$inefficiency <- runif(N * length(sources), public_inefficiency_min, public_inefficiency_max)
```
...
Seeing that it is difficult to see the nature of the distribution of cost in the violin plots above, let us also depict that property as a frequency plot.

```{r, summary-polys, echo=FALSE, warning=FALSE, message=FALSE}
phases %>%
  filter(phase %in% DEVELOPMENT_PHASES) %>%
  ggplot(aes(cost/10^6, color=src, fill=src)) +
  geom_freqpoly() +
  facet_wrap(phase ~ ., scale='free') +
...
The table below summarizes the valuation metrics from the start of pre-clinical, in billions
