...
## New assumptions
To perform our analysis (comparing the cost of direct and indirect funding, as well as alternative payout phases for indirect funding) we require two additional parameters.
...
Second, an inefficiency parameter that will be used to measure the fact that many argue that agents with no expectation of monetary returns (e.g. states) are less efficient.
...
data.frame(parameter = c('Public discount rate (%)',
                         'Public inefficiency (multiplier)'),
           min = c(public_discount_rate_min * 100,
                   public_inefficiency_min),
           max = c(public_discount_rate_max * 100,
                   public_inefficiency_max)) %>% kable
```

## Interventions distribution
We apply five interventions (treatments) to our base data sets and later analyze their effects on the valuation metrics.
...
sertkaya2014 <- sertkaya2014 %>%
  ...
  mutate(priv_discount_rate = rtriangle2(1, min, max, mid)) %>%
  ungroup()
```
...
driveab2018 <- driveab2018 %>%
  ...
  group_by(subject) %>%
  mutate(priv_discount_rate = runif(1,
                                    driveab2018_priv_discount_rate_min,
                                    driveab2018_priv_discount_rate_max)) %>%
  ungroup %>%
  ...
  mutate(time = time / 12,     # months -> years
         ...
         priv_discount_rate = priv_discount_rate / 100 # percentage -> fraction
         ) %>%
...
## New assumptions sample
TODO: ADD MORE EXPLANATORY TEXT HERE.
...
add_metrics <- c('enpv_dir', 'enpv_ind')
```
...
agents <- data.frame(
  subject = subjects,
  publ_inefficiency = runif(length(subjects), public_inefficiency_min, public_inefficiency_max),
  publ_discount_rate = runif(length(subjects), public_discount_rate_min, public_discount_rate_max))
...
The question we want to explore in the analysis is whether it is cheaper for the benefactor to directly or indirectly fund antibiotics R&D.
...
Programatically, we use the function below to compute the valuation metrics from all phases to all phases, given some input dataset.

```{r, valuation-function}
# NOTE: The column 'order' is required so that the function knows how to order
# rows of a given subject.
valuation <- function(df, vantage_points=c('PC','P1','P2','P3','P4','M1')) {
  df %>%
    # Cartesian product merge (copy each row per vantage point)
    merge(., data.frame(from = factor(vantage_points, levels=PHASES, ordered=TRUE))) %>%
    # Valuation is subject and vantage point specific
    group_by(src, intervention, subject, from) %>%
    # Remove events that happened before vantage point
    filter(phase >= from) %>%
    # Ensure order is based on order column
    arrange(order) %>%
    # Valuation
    mutate(# subject info
           dev_prizes = sum(prizes), # total prizes available
           dev_time = sum(ifelse(phase < 'M1', time, 0)), # time to market
           dev_prob = prod(ifelse(phase < 'M1', prob, 1)), # prob to market
           # helpers
           ...
           ev = cashflow * prob_to,
           pv = cashflow / ((1 + priv_discount_rate) ^ time_to),
           epv = pv * prob_to,
           # cumulatives
           cum = cumsum(cashflow),
           env = cumsum(ev),
           npv = cumsum(pv),
           enpv = cumsum(epv), # private enpv
           # additional
           enpv_ind = cumsum((-prizes / ((1 + publ_discount_rate) ^ time_to)) * prob_to),
           enpv_dir = cumsum((-cost / ((1 + publ_discount_rate) ^ time_to)) * prob_to)) %>%
...
## Phase-based method
The phase-based method is very straight forward since it assumes that all cashflows related to a phase occur immediately upon phase entry.
We can therefore apply our valuation function to the original dataset directly.

```{r, phase-based-valuation}
phasely_valuation <-
  # Add order column before passing to valuation function
  valuation(intervened %>% mutate(order = phase)) %>%
  # Remove temp order column
  select(-order)
```
  ...
  for (x in 1:ceiling(max(phs$time) + 1)) { # Longest phase
    ...
    year <-
      tibble(src                = phs$src,
             ...
             prizes,
             prob,
             publ_inefficiency  = phs$publ_inefficiency,
             publ_discount_rate = phs$publ_discount_rate,
             priv_discount_rate = phs$priv_discount_rate
...
# Compute absolute time to phases
yearly_valuation <-
  valuation(years %>%
            group_by(src, intervention, subject) %>%
            arrange(phase, phase_year) %>%
            mutate(order = cumsum(time) - time)) %>%
  # Compute year column
  mutate(year = round(time_to)) %>%
  # Remove temp order column
  select(-order)
```
...
First, to sanity check that our samples seem to be properly drawn from their respective distributions.
Then, to explore what valuations we reach using our different valuations methods and metrics.

Unless otherwise specified, this chapter presents statistics based on the "control" sample, i.e. **with no** interventions or inefficiencies applied.
...
phases %>%
  filter(phase %in% DEVELOPMENT_PHASES) %>%
  ggplot(aes(phase, priv_discount_rate*100, color=src, fill=src)) +
  geom_violin() +
  ylab('Discount rate (%)') +
  xlab('Phase') +
  theme(legend.position = 'top')
...
## Prize sizes
The histogram below depict our sampled intervention prizes at their corresponding phases.
...
## Phase-based summary
Let us now look at some summary statistics of using the phase-based method.

```{r, phase-based-wrangling, echo=FALSE}
# Long
phasely_valuation_long <- phasely_valuation %>%
  select(src, intervention, subject, from, phase, dev_prizes,
         cashflow, ev, pv, epv,
         cum, env, npv, enpv,
         enpv_dir, enpv_ind) %>%
  gather('metric', 'value', -src, -intervention, -subject, -from, -phase, -dev_prizes) %>%
  transform(metric = factor(metric, levels = c(metrics, cum_metrics, add_metrics)))

# Final valuations
phasely_final_valuation <- phasely_valuation %>%
  group_by(src, intervention, subject, from, dev_prizes) %>%
  arrange(phase) %>%
...
phasely_final_valuation_long <- phasely_valuation_long %>%
  filter(metric %in% c(cum_metrics, add_metrics)) %>%
  group_by(src, intervention, subject, from, metric, dev_prizes) %>%
  arrange(phase) %>%
  summarize(value = tail(value, n=1)) %>%
  ungroup
```

Let us first plot the value of taking the project from beginning (i.e. pre-clinical) to end (i.e. the final market year), using our cumulative valuation metrics.
...
phasely_final_valuation_long %>%
  filter(intervention == 'NONE' & metric %in% cum_metrics) %>%
...
## Year-based summary
Let us now look at some summary statistics of using the year-based method.

```{r, year-based-wrangling, echo=FALSE}
# Long
yearly_valuation_long <- yearly_valuation %>%
  select(src, intervention, subject, from, year, dev_prizes,
         cashflow, ev, pv, epv,
         cum, env, npv, enpv,
         enpv_dir, enpv_ind) %>%
  gather('metric', 'value', -src, -intervention, -subject, -from, -year, -dev_prizes) %>%
  transform(metric = factor(metric, levels = c(metrics, cum_metrics, add_metrics)))

# Final valuations
yearly_final_valuation <- yearly_valuation %>%
  group_by(src, intervention, subject, from, dev_prizes, dev_time, dev_prob) %>%
  arrange(year) %>%
  summarize(cum = tail(cum, n=1),
            env = tail(env, n=1),
            npv = tail(npv, n=1),
...
## Comparing methods
Let us now compare the two methods (phase-based and year-based) by looking at the cumulative valuations from different starting points.
...
# Analysis
To prepare for "within-subject" analysis we first select all valuation columns and then match every intervention (including the control group, i.e. no intervention) with every intervention.
Then we remove "duplicated" rows, which appear since every intervention (e.g. P1ER) will not only be matched with every other intervention but also with itself, as these are irrelevant for analysis.

```{r, comparable-years}
finals <- yearly_final_valuation %>%
  # Only consider valuations from first phase
  filter(from == 'PC') %>%
  # Only keep identifying, characteristics, and valuation columns
  select(src, intervention, subject, dev_prizes, dev_prob, dev_time,
         cum, env, npv, enpv, enpv_ind, enpv_dir) %>%
  # Make cartesian product to allow comparing valuations between interventions
  # within subject. NOTE: Merging by more cols than just subject to avoid
  # duplicating cols.  E.g. src would be duplicated into src.x and src.y.
  merge(., ., by=c('src', 'subject', 'dev_prob', 'dev_time')) %>%
  # Remove duplicates since comparing to itself makes no sense
  filter(intervention.x != intervention.y)
```
...
print(finals %>%
  filter(intervention.x == 'NONE') %>%
  ggplot(aes(dev_prizes.y, enpv.y - enpv.x, col=intervention.y)) +
  geom_point(alpha=1, shape=1) +
  #geom_smooth(method='lm', se=FALSE, col='black') +
  facet_wrap(intervention.y ~ src, ncol=7, scale='free') +
  xlab('Prize') +
  ylab('Private ENPV') +
...
print(finals %>%
  filter(intervention.x == 'NONE') %>%
  ggplot(aes(dev_prizes.y, enpv.y - enpv.x, color=intervention.y)) +
  geom_smooth(method='lm', se=FALSE) +
  geom_point(shape=1) +
  xlab('Prize') +
...
finals %>%
  filter(intervention.x == 'NONE') %>%
  ggplot(aes(dev_prizes.y, enpv.y >= 0, color=intervention.y)) +
  geom_jitter(shape=1) +
  facet_grid(cols=vars(src), rows=vars(intervention.y)) +
...
finals %>%
  filter(intervention.x == 'NONE' & enpv.x < 0) %>%
  ggplot(aes(dev_prizes.y, enpv.y >= 0, color=intervention.y)) +
  geom_jitter(shape=1) +
  facet_grid(cols=vars(src), rows=vars(intervention.y)) +
  theme(legend.position = 'none') +
...
```{r, predict-go-in-nos}
# Only no-decisions in control group before treatment
nos <- finals %>% filter(enpv.x < 0 &
                         intervention.x == 'NONE')

go_model <- glm(
  (enpv.y >= 0) ~ dev_prizes.y * interaction(src, intervention.y),
  data = nos,
  family = binomial(link='logit'))

nos$pred.go.y <- predict(go_model, type='response')
```
...
nos %>%
  ggplot(aes(dev_prizes.y, pred.go.y*100, color=intervention.y)) +
  geom_line() +
  scale_x_continuous(trans = 'log',
                     breaks = c(1 %o% 10^(0:10)),
                     labels = trans_format('log10', math_format(10^.x))) +
...
```{r, predict-go}
mod <- glm(
  (enpv.y >= 0) ~ dev_prizes.y * interaction(src, intervention.x, intervention.y),
  data = finals,
  family = binomial(link='logit'))

finals$pred.go.y <- predict(mod, type='response')
```
...
finals %>%
  ...
  filter(intervention.x == 'NONE') %>%
  ggplot(aes(dev_prizes.y, -enpv_ind.y, color=intervention.y)) +
  geom_point(shape=1) +
  geom_smooth(method='lm', se=FALSE) +
...
finals %>%
  filter(intervention.x == 'NONE' & enpv.y >= 0) %>%
  ggplot(aes(dev_prizes.y, -enpv_ind.y, color=intervention.y)) +
  geom_point(shape=1) +
  geom_smooth(method='lm', se=FALSE) +
  facet_wrap(. ~ src, ncol=4) +
...
finals %>%
  filter(intervention.x == 'NONE' & enpv.y >= 0) %>%
  mutate(enpv_ind_per_exit.y = enpv_ind.y/dev_prob) %>%
  ggplot(aes(dev_prizes.y, -enpv_ind_per_exit.y, color=intervention.y)) +
  geom_smooth(method='lm', se=FALSE) +
...
finals %>%
  filter(intervention.x == 'NONE' & pred.go.y < go_lim_up) %>%
  ggplot(aes(pred.go.y*100, ifelse(enpv.y >= 0, -enpv_ind.y, 0), color=intervention.y)) +
  geom_point(shape=1) +
  ...
  scale_y_continuous(label = unit_format(scale = 1e-6, unit = '')) +
  theme(legend.position='none') +
  facet_grid(cols=vars(intervention.y), rows=vars(src))

finals %>%
  filter(pred.go.y < go_lim_up) %>%
  filter(enpv.y >= 0) %>%
  ggplot(aes(pred.go.y*100, ifelse(enpv.y >= 0, -enpv_ind.y, 0), color=intervention.y)) +
  geom_point(shape=1) +
  geom_smooth(method='loess', se=FALSE, color='black') +
...
b <- data.frame(finals) %>%
  ...
  mutate(per = 'per PC entry',
         enpv_ind.y = ifelse(enpv.y >= 0, enpv_ind.y, 0))

rbind(a, b) %>%
  ggplot(aes(pred.go.y*100, -enpv_ind.y, color=intervention.y, linetype=per, shape=per)) +
  #geom_point() +
...
finals %>%
  filter(intervention.x == 'NONE' &
         pred.go.y < go_lim_up &
         enpv.y >= 0) %>%
  ggplot(aes(pred.go.y*100, -enpv_ind.y/dev_prob, color=intervention.y)) +
  geom_smooth(method='loess', se=FALSE) +
  facet_wrap(. ~ src, scales='free', nrow=2) +
  xlab('Probability of go-decision (%) under prize size') +
