...
Highest p-value is lower than 9^`r ends$max` and lowest lower than 10^`r ends$min`.
...
ggsave(file = './_figures/predict-dir.eps', plot = last_plot())
...
ggsave(file = './_figures/predict-dir-per-src.eps', plot = last_plot())
```
...
`r # 10 takes ~ 34 minutes`
```{r, V, echo=FALSE}
V = 20 # TODO: More?
pgos <- seq(0.5, 0.95, length.out=37)
inefs <- seq(0, 1, length.out=21)
```
...
```{r, resampled-heatmap-per-go, echo=FALSE, dependson=c('resampled'), fig.fullwidth=TRUE, fig.width=10, fig.height=12}
f <- function (x) filter(x, minima)
tmp <- resampled %>%
  # Only compare cost diff in projs with go-decison after intervention
  filter(int_priv_enpv >= 0) %>%
  mutate(diff = inef_dir_enpv - ind_enpv) %>%
  ...
  mutate(mean_diff = mean_diff/10^6) %>%
  # Find point closest to 0, i.e. where there is no diff
  group_by(src, prize_phase, publ_inefficiency) %>%
  mutate(minima = near(abs(mean_diff), min(abs(mean_diff))) & min(mean_diff) < 0 & max(mean_diff) > 0) %>%
  ungroup
tmp %>%
  ggplot(aes(publ_inefficiency, pred_go, z=abs(mean_diff), fill=abs(mean_diff))) +
  facet_grid(src ~ prize_phase) +
  geom_raster(interpolate=TRUE) +
  geom_contour(binwidth=25, size=0.1, color='black') +
  geom_smooth(data = f, aes(publ_inefficiency, pred_go),
              color = 'white', size = 0.5,
              se = FALSE,
              method = 'nls',
              method.args = list(formula = y ~ k + a*x + b*x^2,
                                 start = c(k=0.5, a=1, b=-1))) +
  scale_fill_viridis_c(direction = -1,
                       breaks = pretty_breaks(n=10),
                       values = rescale(c(0, max(abs(tmp$mean_diff)))),
                       option = 'viridis',
                       name = 'Mean cost savings (million USD)',
                       guide = guide_colorbar(barwidth=30, title.position='top')
                         ) +
  scale_x_continuous(breaks = pretty_breaks(n=11)) +
  scale_y_continuous(breaks = pretty_breaks(n=11)) +
  ylab('P(go)') +
  ...
  theme(axis.text.x = element_text(angle=90, vjust=0.5, hjust=1),
        ...
        legend.position = 'top',
        panel.spacing = unit(0, 'mm'))
ggsave(file = './_figures/cost-savings-heatmap-per-go.eps', plot = last_plot())
```

```{r, resampled-heatmap-per-me, echo=FALSE, dependson='resampled', fig.fullwidth=TRUE, fig.width=10, fig.height=12}
tmp <- resampled %>%
  # Only compare cost diff in projs with go-decison after intervention
  filter(int_priv_enpv >= 0) %>%
  mutate(diff = inef_dir_enpv/prob_to - ind_enpv/prob_to) %>%
  group_by(src, prize_phase, prize_size, pred_go, publ_inefficiency) %>%
  summarize(mean_diff = mean(diff)) %>%
  mutate(mean_diff = mean_diff/10^6) %>%
  # Find point closest to 0, i.e. where there is no diff
...
tmp %>%
  ...
  facet_grid(src ~ prize_phase) +
  geom_raster(interpolate=TRUE) +
  geom_contour(binwidth=500, size=0.1, color='black') +
  geom_smooth(data = f, aes(publ_inefficiency, pred_go),
              color = 'white', size = 0.5,
              se = FALSE,
              method = 'nls',
  ...
  theme(axis.text.x = element_text(angle=90, vjust=0.5, hjust=1),
        ...
        legend.position = 'top',
        panel.spacing = unit(0, 'mm'))
ggsave(file = './_figures/cost-savings-heatmap-per-me.eps', plot = last_plot())
```

### Limits
```{r, resampled-heatmap-limits-per-me, echo=FALSE, dependson='resampled', fig.fullwidth=TRUE, fig.width=10}
tmp <- resampled %>%
  # Only compare cost diff in projs with go-decison after intervention
  filter(int_priv_enpv >= 0) %>%
  mutate(diff = inef_dir_enpv/prob_to - ind_enpv/prob_to) %>%
  ...
  f
tmp %>%
  ggplot(aes(publ_inefficiency, pred_go, z=mean_diff, color=src)) +
  facet_grid(~ prize_phase) +
  geom_smooth(aes(publ_inefficiency, pred_go),
              size = 0.5,
              se = FALSE,
              method = 'nls',
              method.args = list(formula = y ~ k + a*x + b*x^2,
...
ggsave(file = './_figures/cost-savings-limits-per-me.eps', plot = last_plot())
```
...
scenario %>%
  ...
  theme(legend.position = 'top',
        ...
        axis.text.y = element_blank(),
        axis.ticks.y = element_blank())
ggsave(file = './_figures/scenario-hist.eps', plot = last_plot())
```
...
scenario %>%
  ...
  theme_minimal() +
  theme(legend.position = 'top')
ggsave(file = './_figures/scenario-costs.eps', plot = last_plot())
```
