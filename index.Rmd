...
to_years <- function (phases) {
  ...
  phase_years <- tibble()
  for (x in 1:ceiling(max(phases$time) + 1)) { # Longest phase
...
intervened <- phases %>%
  # TODO: This should not be necessary. Add interventions before you perform
  # valuation. This means you avoid the whole issue of having to value twice.
  select(-intervention, -prizes) %>%
