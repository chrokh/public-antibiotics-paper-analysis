...
market_size <-
  ...
  select(indication, subject, size)

market_share <-
  # Replicate distributions N times
  do.call('rbind', replicate(N * num_indications, share_dist, simplify=FALSE)) %>%
  # Add subject identifier
  mutate(subject = rep(sertkaya2014_subjects, each = nrow(share_dist)))

sertkaya2014_market <-
  # Left join
  merge(market_share, market_size, by='subject', x.all=TRUE) %>%
  # Sample market share
  mutate(share = runif(n(), min, max)) %>%
  ...
  select(subject, src, phase, time, cost, prob, share, size, sales)
```
