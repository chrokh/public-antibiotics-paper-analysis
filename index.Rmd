...
pc <- data.frame(
  ...
  cost  = runif(N, min=19, max=23.2) * 10^6,
...
p1 <- data.frame(
  ...
  cost  = runif(N, min=7.3, max=12) * 10^6,
  sales = 0)
p2 <- data.frame(
  phase = 'P2',
  time  = runif(N, min=0.75, max=2.5),
  cost  = runif(N, min=7.12, max=18.72) * 10^6,
  prob  = runif(N, min=0.34, max=0.74),
  sales = 0)
p3 <- data.frame(
  phase = 'P3',
  cost  = runif(N, min=26.88, max=121.68) * 10^6,
...
mp <- data.frame(
  ...
  prob  = 1,
  cost  = 0,
  sales = runif(N, min=218, max=2500) * 10^6)
...
pc <- data.frame(
  ...
  cost  = rtriangle(N, 14.25, 29, 21.1) * 10^6,
...
p1 <- data.frame(
  ...
  cost  = rtriangle(N, 13.1, 37.96, 24) * 10^6,
  sales = 0)
p2 <- data.frame(
  phase = 'P2',
  time  = rtriangle(N, 0.75, 2.5, 1.08),
  prob  = rtriangle(N, 0.34, 0.74, 0.5),
  cost  = rtriangle(N, 4.55, 46.36, 12.95) * 10^6, # TODO: Cost is incorrectly reported in DRIVE-AB report!
  sales = 0)
p3 <- data.frame(
  phase = 'P3',
  time  = rtriangle(N, 0.83, 3.83, 1.82),
  prob  = rtriangle(N, 0.314, 0.786, 0.67),
  cost  = rtriangle(N, 10, 47, 21.8) * 10^6,
  sales = 0)
p4 <- data.frame(
  phase = 'P4',
  time  = rtriangle(N, 0.5, 1.04, 0.75),
  prob  = rtriangle(N, 0.83, 0.99, 0.85),
  cost  = rtriangle(N, 55.5, 127.91, 88.35) * 10^6,
  sales = 0)
mp <- data.frame(
  phase = 'MP',
  time  = 10,
  prob  = 1,
  cost  = 0,
  sales = rtriangle(N, 0, 4336.00, 2559.5) * 10^6) # TODO: Does not model DRIVE-AB report correctly as the report does not assume that y1 has 0 sales.
...
```{r, sample-inefficiency}
phases$inefficiency <- runif(N, 0, 3)
```
...
p1 <- ggplot(phases, aes(cost/10^6, color=src, fill=src)) +
  geom_freqpoly() +
  ...
  scale_x_continuous(breaks = scales::pretty_breaks(n = 8),
                     #label = unit_format(scale=1e-6)
                     #label = dollar_format()
                     ) +
                     #label = unit_format(unit = 'M')) +
...
p3 <- ggplot(phases, aes(prob*100, color=src, fill=src)) +
  ...
  xlab('Probability (%)')
p4 <- ggplot(phases, aes(sales / 10^9, color=src, fill=src)) +
  geom_freqpoly() +
  facet_grid(phase ~ ., scale='free_y') +
  theme(axis.title.y=element_blank(),
        axis.text.y=element_blank(),
...
```{r, summary-means, echo=FALSE, warning=FALSE, message=FALSE}
for (curr in sources) {
...
phasely_final_from_phases_long %>%
  ...
  ggplot(aes(valuation, value / 10^6, fill=src)) +
  geom_boxplot() +
  ylab('USD (millions)') +
  xlab(element_blank()) +
...
```{r, phase-based-summary2, echo=FALSE}
...
phasely_final_from_phases_long_sum %>%
  ...
  xlab(element_blank()) + ylab('USD (millions)')
```
...
```{r, year-based-conversion, results='hide'}
...
```{r, combine-methods, echo=FALSE, warning=FALSE}
...
```{r, methods-comparison1, echo=FALSE}
p <- both %>%
...
```{r, methods-comparison2, echo=FALSE}
...
prize_samples <- log10_sample(N, 1, 9.999, -1, 3) * 10^6 # Reuse intervention samples across phases
...
finals <- comparable_years %>%
  arrange(t) %>%
  group_by(src, subject, intervention, inefficiency, tot_prob) %>%
  # TODO: This is basically "last row per group". Refactor!
  summarise(
         prizes = sum(prizes),
...
finals %>%
  ...
  scale_y_continuous(trans = 'log',
                     ...
                     label = unit_format(unit = '')) +
  xlab('Prize (million usd)') +
  ylab('-Indirect ENPV per entry (including terminations)\n(million usd)') +
  theme(legend.position = 'top')
```
...
finals %>%
  ...
  scale_x_continuous(trans = 'log',
                     ...
                     label = unit_format(scale = 1e-6, unit = '')) +
  scale_y_continuous(trans = 'log',
                     breaks = c(1 %o% 10^(0:3)) * 10^6,
                     minor_breaks = c(1:9 %o% 10^(0:3)) * 10^6,
                     label = unit_format(scale = 1e-6, unit = '')) +
  xlab('Prize \n(million usd)') +
...
We can also "flip" the data and look at the cost per output rather than the cost per input.
When doing this we must again only look at the projects that reach go-decisions, as we will not pay for the ones that don't.
This still however suffers from the issues outlined above.
We are not taking the probability of any given project facing a go-decision into account, meaning that the cost of low late-phase prizes may appear cheaper since the projects that they stimulate are systematically different (i.e. they are the "cheap" ones).
...
As argued above, we will use the full dataset that includes projects that would have reached go-decisions irrespective of the existance of an intervention as ignoring them would systematically skew the data.
