...
We begin by computing the difference between indirect ENPV and direct ENPV and again plot that for every go-rate (i.e. for every prize, expressed as a probability of go-decision).
Note that at lower go-rates the difference is less but at higher go-rates the difference is greater than zero.
This suggests that whether direct or indirect funding is cheaper, depends on what go-rate you want to achieve.
This is unsuprising since direct funding by definition entails a go-rate of 100%.
Using loess smoothing we again clearly observe the "layering" of the interventions.

```{r, echo=FALSE}
finals %>%
  filter(go_pred_prize < go_lim_up) %>%
  ggplot(aes(go_pred_prize*100, enpv_dir-enpv_ind, color=intervention)) +
  geom_point(shape=1) +
  geom_hline(aes(yintercept=0)) +
  facet_grid(rows=vars(src), cols=vars(intervention)) +
  #facet_wrap(. ~ src) +
  xlab('Probability of go-decision (%)') +
  ylab('Indirect minus direct (i.e. ENPV savings by paying directly)') +
  theme(legend.position = 'top')
...
The above comparison however, assumes that the public and the private sector are equally efficient.
We will therefore now explore how this cost difference vary under various reductions in public sector efficiency.
