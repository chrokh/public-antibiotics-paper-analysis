...
```{r, echo=FALSE}
# Compile time: 2h 30 min.
# Compile time: ~7h 30 min (with increased heatmap samples).
```
...
# Take m samples from dataset df
...
sci <- function (d) format(d, scientific=TRUE)
```
...
```{r, valued_cum, echo=FALSE}
valued_long <-
  valued %>%
  select(subject, src, time_to,
         priv_ccf,
         priv_env,
         priv_npv,
         priv_enpv) %>%
  pivot_longer(cols = c(priv_ccf, priv_env, priv_npv, priv_enpv),
               names_to = 'metric',
               values_to = 'value')
```

Filtered to only display the first 30 years.

```{r, valuation-summary, echo=FALSE, dependson=valued_long}
valued_long %>%
  mutate(time_to = round(time_to),
         value = value / 10^6) %>%
  #filter(time_to <= 30) %>%
  group_by(metric, time_to) %>%
  summarize(value_mean = mean(value),
            value_upper = mean(value) + sd(value),
            value_lower = mean(value) - sd(value)) %>%
  ungroup %>%
  mutate(metric = factor(metric,
                         levels = c('priv_ccf', 'priv_env', 'priv_npv', 'priv_enpv'),
                         labels = c('Cashflow', 'Cumulative Expected Value', 'Net Present Value (NPV)', 'Expected Net Present Value (ENPV)'),
                         ordered = TRUE)) %>%
  ggplot(aes(time_to, value_mean)) +
  geom_line() +
  geom_line(aes(y = value_lower), linetype='dashed') +
  geom_line(aes(y = value_upper), linetype='dashed') +
  facet_wrap(. ~ metric, scale='free_y') +
  ylab('Valuation (million USD)') +
  xlab('Year') +
  scale_x_continuous(limits = c(0, 30)) +
  theme_light(base_size = 11) +
  theme(strip.text = element_text(colour = 'black'),
        strip.background = element_rect(fill = 'white')) # lightgray fill?

ggsave(file = './_figures/thesis/valuation.pdf')
```
...
improvement_models %>%
  unnest(c(data, pred_improvement)) %>%
...
go_models %>%
  ...
  scale_x_continuous(trans = 'log',
                     breaks = 10^(0:10),
                     minor_breaks = c(2:9 %o% 10^(5:10)),
                     labels = trans_format('log10', math_format(10^.x))) +
  scale_y_continuous(breaks = pretty_breaks(n = 11))


ggsave(file = './_figures/fig-01.eps', width=20, units='cm')
ggsave(file = './_figures/fig-01.tiff', width=20, units='cm', dpi=300, scale=1, compression='lzw')
ggsave(file = './_figures/fig-01.jpg', width=20, units='cm')
```

```{r, kjhlsdfsd-bw, echo=FALSE}
go_models %>%
  unnest(c(data, pred_go)) %>%
  ggplot(aes(prize_size, pred_go, linetype=prize_phase)) +
  geom_line() +
  ...
  scale_x_continuous(trans = 'log',
                     ...
                     minor_breaks = c(2:9 %o% 10^(5:10)),
                     labels = trans_format('log10', math_format(10^.x)),
                     limits = c(10^5.5, 10^10.5)) +
  scale_y_continuous(breaks = pretty_breaks(n = 6))

ggsave(file = './_figures/thesis/pgo.pdf', height=9)
```
...
ps <- tidied %>% select(p.value)
```

Highest p-value is lower than 10^`r ends$max` and lowest lower than 10^`r ends$min`.

Highest p-value is `r sci(max(ps))` and lowest is `r sci(min(ps))`.
...
```{r, sanity-check-pred-go, echo=FALSE}
...
```{r, ashjldfsdj, echo=FALSE, dependson=-1}
max_prob <- 0.99999
tmp %>%
...
```{r, kjsdfhlksjdf, echo=FALSE, dependson=-1}
tmp %>%
  filter(pred_go < max_prob) %>%
...
```{r, sdlfhsdklfhsd, echo=FALSE, dependson=-1}
tmp %>%
...
```{r, effectives_target, echo=FALSE}
target_p_go <- 0.9
```

Using our function that solves for prize size from a given probability of go we can predict what prize sizes are required to achive a target probability of go of `r target_p_go*100`% in each of the sources?

```{r, effectives, dependson='effectives_target'}
effectives <- tidied %>%
  group_by(src, prize_phase) %>%
  nest %>%
  mutate(pred_prize_size = map2_dbl(data, target_p_go, predict_prize_from_prob)) %>%
  select(-data) %>%
  ungroup
```

```{r, alslflhjsdfkjhs, echo=FALSE, dependson=-1}
effectives %>%
  ...
  ggtitle(sprintf('P(go) = %s%%', target_p_go*100))
```

If prizes are not targeted by indication we must choose the highest prize to ensure a minimum of `r target_p_go*100`% go-decisions for all indications.
Hence the "max" row.

```{r, hjklfsdf, echo=FALSE, dependson=-1}
effectives %>%
  ...
  kable(caption=sprintf('Million usd (rounded). P(go) = %s%%.', target_p_go*100))
```
...
Highest and lowest p-values are `r sci(max(ps))` and `r sci(min(ps))` respectively.

Lowest and highest predicted values are `r sci(min(preds))` and `r sci(max(preds))` respectively.
...
dir_models %>%
  unnest_legacy(data, pred_dir) %>%
  ggplot(aes(publ_inefficiency, pred_dir, color=prize_phase)) +
    geom_point(shape=1) +
    facet_wrap(. ~ src) +
    xlab('Public inefficiency') +
    ylab('P(direct)') +
...
```{r, dir_models_with_inef_output, echo=FALSE, dependson=-1}
dir_models %>%
  select(src, prize_phase, tidied_dir) %>%
  unnest(tidied_dir) %>%
  mutate(p.value = sci(p.value)) %>%
  kable
```
...
Lowest and highest predicted values are `r sci(min(preds))` and `r max(preds)` respectively.

Prepare function for predicting P(go) from inefficiency and target P(direct):
```{r, dir_models_backwards_pred, dependson=-1}
predict_p_go_from_prob_and_inef <- function(tidied, x2, y) {
  b0 <- filter(tidied, term=='(Intercept)')$estimate
  b1 <- filter(tidied, term=='pred_go')$estimate
  b2 <- filter(tidied, term=='publ_inefficiency')$estimate
...
steps <- 10
inefficiencies <-
  merge(data.frame(src = INDICATIONS),
        data.frame(publ_inefficiency = seq(0, 1, length.out=steps)))
evens <- dir_models %>%
  select(src, prize_phase, tidied_dir) %>%
  unnest(tidied_dir) %>%
  merge(., inefficiencies) %>%
  group_by(src, prize_phase, publ_inefficiency) %>%
  nest %>%
  mutate(pred_pred_go = mapply(predict_p_go_from_prob_and_inef, data, publ_inefficiency, target_dir)) %>%
  unnest(pred_pred_go) %>%
  ungroup %>%
  mutate(pred_dir = target_dir) # Needed for z in heatmaps
```
...
# Zoomed in to scenario
p +
  coord_cartesian(ylim=c(0.895, 0.905), xlim=c(0.4, 0.6)) +
  geom_hline(yintercept=0.9, linetype='dashed') +
  geom_vline(xintercept=0.5, linetype='dashed') +
  ggtitle('Zoomed in to scenario')
...
`r # NOTE: 10 takes ~ 453 minutes (~8h).`
```{r, V, echo=FALSE}
V = 10
pgos <- seq(0.5, 0.9875, length.out=40)
inefs <- seq(0, 1, length.out=41)
```
Mean cost savings of direct funding, computed by resampling:

- `r length(pgos)` evenly spaced prize sizes yielding P(go) between `r min(pgos)*100`% and `r max(pgos)*100`%.
- `r length(inefs)` evenly spaced inefficiencies between `r min(inefs)*100`% and `r max(inefs)*100`%.

Takes `r V` first samples for each above combination from the original sample set.

```{r, resampled, echo=FALSE, dependson=c('V')}
models <- go_models %>%
  group_by(src, prize_phase) %>%
  select(src, prize_phase, tidied)
...
hardcoded =
  ...
  merge(., intervention_phase_dist) %>%
  # Add P(go)
  merge(., data.frame(pred_go = pgos)) %>%
  # Add inefficiency
  merge(., data.frame(publ_inefficiency = inefs)) %>%
  # Merge go models
  merge(., models, by=c('src', 'prize_phase')) %>%
  # Solve for prize_size from P(go)
  #mutate(prize_size = map2(tidied, pred_go, predict_prize_from_prob)) %>%
  mutate(prize_size = mapply(predict_prize_from_prob, tidied, pred_go)) %>%
...
resampled <- valuable %>%
  ...
  select(-publ_inefficiency, -prize_size) %>%
  # Replace with spaced out range
  merge(., hardcoded, by=c('src', 'prize_phase')) %>%
  # Rebuild subject identifier since we've now duplicated subjects
  mutate(subject = group_indices(., subject, prize_size, publ_inefficiency)) %>%
  ## Run valuation but only keep final
  valuation %>%
  final_valuation
```

```{r, resampled-pgo-heatmaps, echo=FALSE, dependson='resampled', fig.fullwidth=TRUE, fig.width=10, fig.height=12}
resampled %>%
  # Only compare cost diff in projs with go-decison after intervention
  filter(int_priv_enpv >= 0) %>%
  mutate(diff = inef_dir_enpv - ind_enpv,
         dir = ifelse(diff > 0, 1, 0),
         ind = ifelse(diff > 0, 0, 1)) %>%
  group_by(src, prize_phase, pred_go, publ_inefficiency) %>%
  summarize(pdir = sum(dir) / n()) %>%
  ungroup %>%
  ggplot(aes(publ_inefficiency, pred_go, z=pdir)) +
  facet_grid(src ~ prize_phase) +
  geom_tile(aes(fill=pdir)) +
  scale_x_continuous(breaks = pretty_breaks(n=11)) +
  scale_y_continuous(breaks = pretty_breaks(n=11)) +
  ylab('P(go)') +
  xlab('Inefficiency') +
  scale_fill_gradient2(low='red', high='blue', mid='white', midpoint=0.5,
                       guide = guide_colorbar(barwidth=25, title.position='top', limits=c(0, 1)),
                       breaks = pretty_breaks(n=11),
                       name='Probability of cost savings when directly funding, by go-decision') +
  theme_minimal() +
...
## Cost difference (resampled)

Compute means, find minimas and fit model to minimas.

```{r, resampled_diff_summary, echo=FALSE, dependson=c('resampled')}
resampled_diff_summary <- resampled %>%
  # Only compare cost diff in projs with go-decison after intervention
  filter(int_priv_enpv >= 0) %>%
  mutate(diff_per_go = inef_dir_enpv - ind_enpv,
         diff_per_me = inef_dir_enpv/prob_to - ind_enpv/prob_to) %>%
  group_by(src, prize_phase, prize_size, pred_go, publ_inefficiency) %>%
  summarize(mean_diff_per_go = mean(diff_per_go),
            mean_diff_per_me = mean(diff_per_me)) %>%
  ungroup %>%
  # Find point closest to 0, i.e. where there is no diff
  group_by(src, prize_phase, publ_inefficiency) %>%
  mutate(minima_per_go = near(abs(mean_diff_per_go), min(abs(mean_diff_per_go))) & min(mean_diff_per_go) < 0 & max(mean_diff_per_go) > 0,
         minima_per_me = near(abs(mean_diff_per_me), min(abs(mean_diff_per_me))) & min(mean_diff_per_me) < 0 & max(mean_diff_per_me) > 0) %>%
  ungroup
```

```{r, resampled_minima_per_go, echo=FALSE, dependson=c('resampled_diff_summary')}
resampled_minima_per_go_model <- function (df)
  nls(pred_go ~ k + a*publ_inefficiency + b*publ_inefficiency^2,
      data=df,
      start=list(k=0.5, a=1, b=-1))
resampled_minima_per_go <- resampled_diff_summary %>%
  filter(minima_per_go) %>%
  group_by(src, prize_phase) %>%
  nest %>%
  mutate(minima_fit = map(data, resampled_minima_per_go_model),
         minima_pred = map2(minima_fit, data, predict_response),
         minima_tidied = map(minima_fit, tidy)) %>%
  ungroup
resampled_minima_per_go %>%
  unnest(c(data, minima_pred)) %>%
  ggplot(aes(publ_inefficiency, pred_go, group=prize_phase, color=prize_phase)) +
  facet_wrap(~src) +
  geom_point(shape=0) +
  geom_line(aes(y=minima_pred), color='black') +
  ggtitle('Predicting y from x in minima per go-decision')

ends <- resampled_minima_per_go %>%
  unnest(minima_tidied) %>%
  mutate(p.value = sapply(p.value, p_lower_than)) %>%
  summarize(max = max(p.value), min = min(p.value))
sprintf('P-values between 10^%s and 10^%s', ends$min, ends$max)
```

```{r, resampled_minima_per_me, echo=FALSE, dependson=c('resampled_diff_summary')}
resampled_minima_per_me_model <- function (df)
  nls(pred_go ~ k + a*publ_inefficiency + b*publ_inefficiency^2,
      data=df,
      start=list(k=0.5, a=1, b=-1))
resampled_minima_per_me <- resampled_diff_summary %>%
  filter(minima_per_me) %>%
  group_by(src, prize_phase) %>%
  nest %>%
  mutate(minima_fit = map(data, resampled_minima_per_me_model),
         minima_pred = map2(minima_fit, data, predict_response),
         minima_tidied = map(minima_fit, tidy)) %>%
  ungroup
resampled_minima_per_me %>%
  unnest(c(data, minima_pred)) %>%
  ggplot(aes(publ_inefficiency, pred_go, group=prize_phase, color=prize_phase)) +
  facet_wrap(~src) +
  geom_point(shape=0) +
  geom_line(aes(y=minima_pred), color='black') +
  ggtitle('Predicting y from x in minima per market entry')
ends <- resampled_minima_per_me %>%
  unnest(minima_tidied) %>%
  mutate(p.value = sapply(p.value, p_lower_than)) %>%
  summarize(max = max(p.value), min = min(p.value))
sprintf('P-values between 10^%s and 10^%s', ends$min, ends$max)
...
```{r, resampled-heatmap-per-go, echo=FALSE, dependson=c('resampled_diff_summary', 'resampled_minima_per_go'), fig.fullwidth=TRUE, fig.width=10, fig.height=12}
resampled_diff_summary %>%
  filter(pred_go <= 0.95) %>% # Inluciding high P(go) essentially makes heatmap a single color
  mutate(mean_diff_per_go = mean_diff_per_go / 10^6) %>%
  ggplot(aes(publ_inefficiency, pred_go)) +
  facet_grid(src ~ prize_phase) +
  geom_raster(aes(fill=abs(mean_diff_per_go)), interpolate=TRUE) +
  geom_contour(aes(z=abs(mean_diff_per_go)), binwidth=25, size=0.1, color='black') +
  geom_line(data = resampled_minima_per_me %>% unnest(c(data, minima_pred)),
            aes(publ_inefficiency, minima_pred),
            color='white', size=0.5) +
  scale_fill_viridis_c(direction = -1,
                       breaks = pretty_breaks(n=10),
                       values = rescale(c(0, max(abs(resampled_diff_summary$mean_diff_per_go)))),
                       option = 'viridis',
                       name = 'Mean cost savings (million USD)',
                       guide = guide_colorbar(barwidth=15, title.position='top')
                         ) +
  scale_x_continuous(breaks = pretty_breaks(n=11)) +
  scale_y_continuous(breaks = pretty_breaks(n=11)) +
  ylab('P(go)') +
...
```{r, resampled-heatmap-per-me, echo=FALSE, dependson=c('resampled_diff_summary', 'resampled_minima_per_me'), fig.fullwidth=TRUE, fig.width=10, fig.height=12}
resampled_diff_summary %>%
  filter(pred_go <= 0.95) %>% # Inluciding high P(go) essentially makes heatmap a single color
  mutate(mean_diff_per_me = mean_diff_per_me / 10^6) %>%
  ggplot(aes(publ_inefficiency, pred_go)) +
  facet_grid(src ~ prize_phase) +
  geom_raster(aes(fill=abs(mean_diff_per_me)), interpolate=TRUE) +
  geom_contour(aes(z=abs(mean_diff_per_me)), binwidth=250, size=0.1, color='black') +
  geom_line(data = resampled_minima_per_me %>%
              unnest(c(data, minima_pred)) %>%
              filter(pred_go <= 0.95),
            aes(publ_inefficiency, minima_pred),
            color='white', size=0.5) +
  scale_fill_viridis_c(direction = -1,
                       ...
                       values = rescale(c(0, max(abs(resampled_diff_summary$mean_diff_per_me)))),
                       option = 'viridis',
                       name = 'Mean cost savings (million USD)',
                       guide = guide_colorbar(barwidth=20, nbin=300, title.position='top')) +
  scale_x_continuous(breaks = pretty_breaks(n=11), expand=c(0,0)) +
  scale_y_continuous(breaks = pretty_breaks(n=11), expand=c(0,0)) +
  ylab('P(go)') +
  xlab('Inefficiency') +
  theme_minimal(base_size=9) +
  theme(axis.text.x = element_text(angle=90, vjust=0.5, hjust=1),
...
ggsave(file = './_figures/fig-02.eps', width=20, height=24, units='cm')
ggsave(file = './_figures/fig-02.tiff', width=20, height=24, units='cm', dpi=300, compression='lzw')
ggsave(file = './_figures/fig-02.jpg', width=20, height=24, units='cm')
```

```{r, resampled-heatmap-per-me-thesis, echo=FALSE, dependson=c('resampled_diff_summary', 'resampled_minima_per_me'), fig.fullwidth=TRUE, fig.width=10, fig.height=12}
resampled_diff_summary %>%
  filter(pred_go <= 0.95) %>% # Inluciding high P(go) essentially makes heatmap a single color
  mutate(mean_diff_per_me = mean_diff_per_me / 10^6) %>%
  ggplot(aes(publ_inefficiency, pred_go)) +
  facet_grid(src ~ prize_phase) +
  geom_raster(aes(fill=abs(mean_diff_per_me)), interpolate=TRUE) +
  geom_contour(aes(z=abs(mean_diff_per_me)), binwidth=250, size=0.1, alpha=0.5, color='black') +
  geom_line(data = resampled_minima_per_me %>%
              unnest(c(data, minima_pred)) %>%
              filter(pred_go <= 0.95),
            aes(publ_inefficiency, minima_pred),
  ...
  scale_fill_viridis_c(direction = -1,
                       #breaks = pretty_breaks(n=10),
                       breaks = seq(0, 6000, by=250), # by=500
                       values = rescale(c(0, max(abs(resampled_diff_summary$mean_diff_per_me)))),
                       option = 'viridis',
                       name = 'Mean cost savings (million USD)',
                       guide = guide_colorbar(barwidth=45.3, nbin=500, title.position='top')) + # bar 20 # nbin 300
  scale_x_continuous(breaks = seq(0, 10, by=1)/10, expand=c(0,0)) +
  scale_y_continuous(breaks = seq(0.5, 1, by=0.05), expand=c(0,0)) +
  ylab('P(go)') +
  xlab('Inefficiency') +
...
ggsave(file = './_figures/thesis/heatmap.pdf', height=12)
```


### Limits
```{r, resampled-heatmap-limits-per-me, echo=FALSE, dependson='resampled_minima_per_me', fig.fullwidth=TRUE, fig.width=10}
resampled_minima_per_me %>%
  unnest(c(data, minima_pred)) %>%
  ggplot(aes(publ_inefficiency, minima_pred, color=src)) +
  facet_wrap(~ prize_phase, nrow=1) +
  geom_line() +
  scale_x_continuous(breaks = pretty_breaks(n=11)) +
  scale_y_continuous(breaks = pretty_breaks(n=11)) +
...
ggsave(file = './_figures/fig-03.eps', width=20, height=10, units='cm')
ggsave(file = './_figures/fig-03.tiff', width=20, height=10, units='cm', compression='lzw')
ggsave(file = './_figures/fig-03.jpg', width=20, height=10, units='cm')
```
...
for (curr_src in unique(diff_models_parametric$src)) {
  for (curr_ph in unique(diff_models_parametric$prize_phase)) {
    sub <- diff_models_parametric %>%
      ...
      mutate(diff_denorm = denormalize(diff_norm, diff),
             pred_diff_pwr_denorm = denormalize(pred_diff_pwr, diff),
             pred_diff_lm_denorm = denormalize(exp(pred_diff_lm), diff))

    p1 <- sub %>%
      ggplot(aes(pred_no, diff_denorm)) +
      facet_wrap(prize_phase ~ src, scale='free') +
      geom_point(aes(color=diff_denorm > 0)) +
      geom_line(aes(y=pred_diff_pwr_denorm), color='red') +
      geom_line(aes(y=pred_diff_lm_denorm), color='blue') +
      scale_x_continuous(breaks = pretty_breaks(n=11)) +
      scale_y_continuous(breaks = pretty_breaks(n=11)) +
...
TODO: Spline log model does not log y. Since the correlation is almost linear on log-log I should definitely try log-y.
...
diff_models_nonparametric <- go_models %>%
  ...
  filter(int_priv_enpv >= 0) %>%
  # Prepare independent variables
  # NOTE: Normalize across groups so denormalization must also happen across.
  mutate(pred_no = 1 - pred_go, # P(no) = 1 - P(go)
         diff = dir_enpv - ind_enpv,
         diff_norm = normalize(diff, diff)) %>%
  group_by(src, prize_phase) %>%
...
diff_models_nonparametric %>%
  ...
  unnest(tidied_diff_spline) %>%
  mutate(p.value = sprintf('< 10^%s', sapply(p.value, p_lower_than))) %>%
  kable(caption = 'Spline')

diff_models_nonparametric %>%
  select(src, prize_phase, tidied_diff_logspline) %>%
  unnest(tidied_diff_logspline) %>%
  mutate(p.value = sprintf('< 10^%s', sapply(p.value, p_lower_than))) %>%
  kable(caption = 'Spline in log')

diff_models_nonparametric %>%
  select(src, prize_phase, tidied_diff_gam) %>%
  unnest(tidied_diff_gam) %>%
  mutate(p.value = sprintf('< 10^%s', sapply(p.value, p_lower_than))) %>%
  kable(caption = 'GAM')
```
...
### Combined plots
```{r, diff_model_nonparametric_combined_figs, echo=FALSE}
sub <- diff_models_nonparametric %>%
  unnest_legacy(data, pred_diff_spline, pred_diff_logspline, pred_diff_gam) %>%
  mutate(pred_diff_logspline_denorm = denormalize(pred_diff_logspline, diff))
p1 <- sub %>%
  ggplot(aes(pred_no, pred_diff_logspline, color=prize_phase)) +
  facet_wrap(. ~ src) +
  geom_line() +
  scale_x_continuous(trans = 'log',
                     breaks = 10^(-2:0),
  ...
  theme(legend.position = 'top')
print(p1)
p2 <- sub %>% ggplot(aes(pred_no, pred_diff_logspline_denorm, color=prize_phase)) +
  facet_wrap(. ~ src) +
  geom_line() +
  scale_x_continuous(trans = 'log',
                     breaks = 10^(-2:0),
...
However, we only keep the `r M` first samples for each source/indication since significantly fewer samples are needed when prize size and public inefficiencies are not stochastic.

```{r, scenario, dependson = c('m', 'effectives')}
scenario <- valuable %>%
  take_subjects(M) %>%
  # Remove old stochastic values
...
### Cost savings (yes/no predicted)
```{r, scenario_dir_expectation-params, echo=FALSE}
pgo = 0.9
inef = 0.5
```

In which phases and indications do we expect direct funding to be cheaper for our scenario? Let's predict using our P(direct) model, when P(go) = `r pgo` and inefficiency = `r inef`.

```{r, scenario_dir_expectation, dependson='dir_models', echo=FALSE}
# TODO: This could most probably be done in a cleaner way
scenario_dir_expectation <- dir_models %>%
  select(src, prize_phase, dir_fit) %>%
  group_by(src, prize_phase) %>%
  # Hard-coded scenario values
  mutate(pred_go = pgo, publ_inefficiency = inef) %>%
  # Predict P(dir)
  nest(data = c(pred_go, publ_inefficiency)) %>%
  mutate(pred_dir = map2_dbl(dir_fit, data, predict_response)) %>%
  unnest(c(data, pred_dir))
```

```{r, scenario-dir-expectation-plot, dependson='scenario_dir_expectation', echo=FALSE}
tmp <- scenario_dir_expectation %>%
  select(src, prize_phase, pred_dir) %>%
  spread(src, pred_dir)
kable(tmp, caption=sprintf('Predicted P(dir) at P(go) = %s and Inefficiency = %s', pgo, inef))

scenario_dir_expectation %>%
  select(src, prize_phase, pred_dir) %>%
  mutate(indirect = 1-pred_dir) %>%
  rename(direct = pred_dir) %>%
  #group_by(src, prize_phase) %>%
  gather('savings', 'prob', -src, -prize_phase) %>%
  ggplot(aes(src, prob, fill=savings)) +
  facet_wrap(. ~ prize_phase, nrow=1) +
  geom_col() +
  geom_hline(yintercept=0.5) +
  scale_y_continuous(breaks = pretty_breaks(n=11)) +
  ...
  xlab(element_blank()) +
  ylab('Predicted probability of cost savings') +
  ggtitle(sprintf('Predicted P(dir) at P(go) = %s and Inefficiency = %s  int_priv_enpv ≥ 0', pgo, inef))
```

### Cost savings (yes/no)
```{r, klhshfks, dependson='scenario', results='asis', echo=FALSE}
tmp <- scenario %>%
  # Only compare cost diff in projs with go-decison after intervention
  filter(int_priv_enpv >= 0) %>%
  group_by(src, prize_phase) %>%
  summarize(dir = sum(ifelse((inef_dir_enpv - ind_enpv) > 0, 1, 0 )),
            ind = sum(ifelse((inef_dir_enpv - ind_enpv) > 0, 0, 1)),
            pdir = dir / n(),
            pind = ind / n()) %>%
  ungroup

tmp %>%
  select(src, prize_phase, pdir) %>%
  arrange(prize_phase) %>%
  spread(src, pdir) %>%
  kable(caption='Resampled P(direct)')

tmp %>%
  select(src, prize_phase, pdir, pind) %>%
  rename(indirect = pind,
         direct = pdir) %>%
  gather('savings', 'prob', -src, -prize_phase) %>%
  ggplot(aes(src, prob, fill=savings)) +
  facet_wrap(. ~ prize_phase, nrow=1) +
  ...
  theme(axis.text.x = element_text(angle=90, vjust=0.5, hjust=1),
        legend.position = 'bottom') +
  xlab(element_blank()) +
  ylab('Ratio of cost savings in resampled scenario') +
  ggtitle('Intervened Private ENPV ≥ 0')
```

### Cost savings (actual)
TODO: Intervened private enpv >= 0 in all below.

```{r, scenario-figs, dependson='scenario', echo=FALSE, results='asis'}
tmp <- scenario %>%
  # Only compare cost diff in projs with go-decison after intervention
  filter(int_priv_enpv >= 0) %>%
  group_by(src, prize_phase) %>%
  ...
  ungroup
tmp %>%
  mutate(mean_diff = round(mean_diff/10^6, 2)) %>%
...
tmp %>%
  ...
  theme(axis.text.x = element_text(angle=90, vjust=0.5, hjust=1),
        legend.position = 'bottom') +
  xlab(element_blank()) +
  ylab('Cost savings of direct funding (million USD)') +
  ggtitle('Narrow prizes (per go-decision)  Intervened Private ENPV >= 0')
```

Where intervened private ENPV >= 0:
```{r, scenario_cost_savings_histogram, dependson='scenario', echo=FALSE, fig.fullwidth=TRUE, fig.width=10, fig.height=2.5}
scenario %>%
  # Only compare cost diff in projs with go-decison after intervention
  filter(int_priv_enpv >= 0) %>%
  group_by(src, prize_phase) %>%
  mutate(dir = inef_dir_enpv - ind_enpv) %>%
  mutate(dir = dir / 10^6) %>%
  ggplot(aes(dir, color=src)) +
  facet_wrap(~ prize_phase, nrow=1, scale='free') +
  geom_freqpoly() +
  geom_vline(xintercept=0, linetype='dotted') +
  ylab('Count') +
  xlab('Cost savings of direct funding (million usd)') +
  scale_x_continuous(breaks = pretty_breaks(n=11)) +
  guides(color = guide_legend(nrow=1, name='')) +
  theme_minimal() +
  theme(legend.position = 'top',
...
#### Per market entry
```{r, scenario-figs-per-market-entry, dependson='scenario', echo=FALSE, results='asis'}
tmp <- scenario %>%
  # Only compare cost diff in projs with go-decison after intervention
  filter(int_priv_enpv >= 0) %>%
  group_by(src, prize_phase) %>%
  ...
  ungroup
tmp %>%
  mutate(mean_diff = round(mean_diff/10^6, 0)) %>%
  spread(prize_phase, mean_diff) %>%
...
tmp %>%
  ...
  xlab(element_blank()) +
  ylab('Cost savings of direct funding (million usd)') +
  ggtitle('Narrow prizes (per market entry)  Intervened Private ENPV >= 0') +
  theme(axis.text.x = element_text(angle=90, vjust=0.5, hjust=1),
        legend.position = 'bottom')
```
...
```{r, scenario-costs, dependson='scenario', echo=FALSE, fig.fullwidth=TRUE, fig.width=10, fig.height=7}
print('per go-decision:')
scenario %>%
  # Only compare cost diff in projs with go-decison after intervention
  filter(int_priv_enpv >= 0) %>%
  mutate(Direct = -inef_dir_enpv,
         Indirect = -ind_enpv) %>%
  select(src, prize_phase, Direct, Indirect) %>%
  gather('funding', 'cost', -src, -prize_phase) %>%
  ggplot(aes(src, cost/10^6, fill=funding)) +
  geom_boxplot(outlier.shape = NA) +
  facet_wrap(.~prize_phase, ncol=1, strip.position='right') +
  scale_y_continuous(breaks = pretty_breaks(n = 15)) +
  xlab(element_blank()) +
  ylab('Cost of funding (million USD)') +
  coord_flip(ylim = c(20, 320)) +
  guides(fill=guide_legend(nrow=1, title=NULL)) +
  theme_minimal() +
  theme(legend.position = 'top')

print('per market entry:')
scenario %>%
  # Only compare cost diff in projs with go-decison after intervention
  filter(int_priv_enpv >= 0) %>%
  mutate(Direct = -inef_dir_enpv / prob_to,
         Indirect = -ind_enpv / prob_to) %>%
  select(src, prize_phase, Direct, Indirect) %>%
  gather('funding', 'cost', -src, -prize_phase) %>%
  ggplot(aes(src, cost/10^6, fill=funding)) +
  geom_boxplot(outlier.shape = NA) +
  ...
  theme_minimal(base_size=9) +
  theme(legend.position = 'top')
ggsave(file = './_figures/fig-04.eps', width=20, height=14, units='cm')
ggsave(file = './_figures/fig-04.jpg', width=20, height=14, units='cm')
ggsave(file = './_figures/fig-04.tiff', width=20, height=14,  units='cm', dpi=300, compression='lzw')
```

```{r, scenario_costs, dependson='scenario', echo=FALSE, fig.fullwidth=TRUE, fig.width=10}
# Below is messy because we're zooming away from outliers
plotf <- function (df, prize_phase, ylab) {
  ytop <- case_when(prize_phase == 'P1' ~ 85,
                    prize_phase == 'P2' ~ 95,
                    prize_phase == 'P3' ~ 140,
                    prize_phase == 'P4' ~ 330,
                    prize_phase == 'M1' ~ 330)
  f <- function (df)
    df %>%
      group_by(src, funding) %>%
      summarize(cost = median(cost)) %>%
      ungroup
  p <- df %>%
    mutate(prize_phase = prize_phase) %>%
    ggplot(aes(src, cost/10^6, fill=funding)) +
    geom_boxplot(outlier.shape = NA) +
    facet_wrap(.~prize_phase, nrow=1) +
    scale_y_continuous(breaks = pretty_breaks(n = 15)) +
    xlab(element_blank()) +
    ylab(element_blank()) +
    coord_cartesian(ylim = c(15, ytop)) +
    guides(fill = guide_legend(nrow=1, title=NULL), color=FALSE) +
    theme_minimal() +
    theme(axis.text.x = element_text(angle=90, vjust=0.5, hjust=1),
          legend.position = 'top')
    if (ylab) p <- p + ylab('Cost of funding (million USD) per go-decision')
    return(p)
}
ps <- scenario %>%
  # Only compare cost diff in projs with go-decison after intervention
  ...
  group_by(prize_phase) %>%
  nest %>%
  mutate(ylab = ifelse(prize_phase == 'P1', TRUE, FALSE),
         plot = pmap(list(data, prize_phase, ylab), plotf)) %>%
  select(prize_phase, plot)
ggarrange(plotlist=ps$plot,
          common.legend=TRUE,
          nrow=1)
...
tmp <- scenario %>%
  ...
  filter(int_priv_enpv >= 0) %>%
  group_by(src, prize_phase) %>%
  summarize(Direct   = mean(-inef_dir_enpv),
            Indirect = mean(-ind_enpv)) %>%
  ungroup %>%
  gather('funding', 'cost', -src, -prize_phase)
tmp %>%
  ggplot(aes(src, cost/10^6, fill=funding)) +
  ...
  scale_y_continuous(breaks = pretty_breaks(n = 15)) +
  xlab(element_blank()) +
  ylab('Cost of funding (million USD) per go-decision') +
  guides(fill = guide_legend(nrow=1, title=NULL)) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle=90, vjust=0.5, hjust=1),
        legend.position = 'top')
tmp %>%
  mutate(cost = round(cost / 10^6)) %>%
  spread(prize_phase, cost) %>%
  kable(caption='per go-decision')

tmp <- scenario %>%
  # Only compare cost diff in projs with go-decison after intervention
  filter(int_priv_enpv >= 0) %>%
  group_by(src, prize_phase) %>%
  summarize(Direct   = mean(-inef_dir_enpv/prob_to),
            Indirect = mean(-ind_enpv/prob_to)) %>%
  ungroup %>%
  gather('funding', 'cost', -src, -prize_phase)
tmp %>%
  ggplot(aes(src, cost/10^6, fill=funding)) +
  ...
  scale_y_continuous(breaks = pretty_breaks(n = 15)) +
  xlab(element_blank()) +
  ylab('Cost of funding (million USD) per market entry') +
  guides(fill = guide_legend(nrow=1, title=NULL)) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle=90, vjust=0.5, hjust=1),
        legend.position = 'top')
tmp %>%
  mutate(cost = round(cost / 10^6)) %>%
  spread(prize_phase, cost) %>%
  kable(caption='per market entry')
```
...
predicted <- span %>%
  ...
  filter(pred_go == target_p_go * 100 & publ_inefficiency == 50) %>%
  select(-pred_go, -publ_inefficiency)

resampled <- scenario %>%
  # Only compare cost diff in projs with go-decision after intervention
  filter(int_priv_enpv >= 0) %>%
  group_by(src, prize_phase) %>%
