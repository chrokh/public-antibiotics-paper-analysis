...
sertkaya2014_market_success_dist <-
  data.frame(indication = INDICATIONS, min = 40, mid = 60, max = 80)
...
kable(sertkaya2014_market_success_dist, caption='sertkaya2014_market_success_dist')
...
# NOTE: This script is dependent on the order of indications remaining the same
# across related data frames.

share_dist <- sertkaya2014_market_share_dist # alias
success_dist <- sertkaya2014_market_success_dist # alias
size_dist <- sertkaya2014_market_size_dist # alias

market_size <-
  # Replicate distributions N times
  do.call('rbind', replicate(N, size_dist, simplify=FALSE)) %>%
  # Add subject identifier
  mutate(subject = sertkaya2014_subjects) %>%
  # Sample market size
  mutate(size = runif(n(), min, max)) %>%
  # Select relevant data
  select(subject, size)

market_success <-
  # Replicate distributions N times
  do.call('rbind', replicate(N, success_dist, simplify=FALSE)) %>%
  # Add subject identifier
  mutate(subject = sertkaya2014_subjects) %>%
  # Sample market success
  mutate(success = rtriangle2(n(), min, max, mid)) %>%
  # Select relevant columns
  select(subject, indication, success)

market_share <-
  # Cartesian product of share dist (all years) and market success samples
  merge(share_dist, market_success) %>%
  # Convert percentages to fractions
  mutate(min=min/100, max=max/100, success=success/100) %>%
  # Compute actual market share as per Table 12 in Sertkaya et. al (2014)
  mutate(share = (min * (1 - success)) + (max * success)) %>%
  # Select relevant columns
  select(subject, indication, year, share)

sertkaya2014_market <-
  # Left join
  merge(market_share, market_size, by='subject', x.all=TRUE) %>%
  # Compute actual yearly sales
  mutate(sales = share * size) %>%
  # Rename columns
  rename(src = indication) %>%
  # Compute phase name
  mutate(phase = paste('M', year, sep='')) %>%
  # Add hard-coded phase data
...
sertkaya2014 <-
  rbind(sertkaya2014_dev, sertkaya2014_market) %>%
