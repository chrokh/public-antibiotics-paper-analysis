...
discount_rate_priv <- runif(N, 0.09, 0.24)
...
pc$discount_rate_priv <- discount_rate_priv
...
p1$sales            <- rep(0, N)
p1$prizes           <- rep(0, N)
p1$discount_rate_priv <- discount_rate_priv
...
p2$discount_rate_priv <- discount_rate_priv
...
p3$sales            <- rep(0, N)
p3$prizes           <- rep(0, N)
p3$discount_rate_priv <- discount_rate_priv
...
p4$sales            <- rep(0, N)
p4$prizes           <- rep(0, N)
p4$discount_rate_priv <- discount_rate_priv
...
mp$prizes        <- 0
mp$time          <- 10
mp$discount_rate_priv <- discount_rate_priv
...
```{r}
discount_rate_priv <- runif(N, 0.05, 0.30) # Same across phases
pc <- data.frame(subject=1:N, phase=factor('PC'))
...
for (from in phase_levels) {
  pyfp <- years %>%
    ...
    mutate(from          = factor(from, levels=phase_levels, ordered=TRUE),
           ...
           cost_pv         = cost / ((1 + discount_rate_priv) ^ time_to),
           cost_epv        = (cost / ((1 + discount_rate_priv) ^ time_to)) * prob_to,
           cost_npv        = cumsum(cost_pv),
           cost_enpv       = cumsum(cost_epv), # TODO:ok?
           # revenue
           revenue_ev      = revenue * prob_to, # TODO:ok?
           revenue_env  = cumsum(revenue_ev),
           revenue_pv      = revenue / ((1 + discount_rate_priv) ^ time_to),
           revenue_epv     = (revenue / ((1 + discount_rate_priv) ^ time_to)) * prob_to,
