...
valuation <- function(df) {
  result <- tibble()
  # Compute valuations from all development phases and first market year
  for (frm in c(DEVELOPMENT_PHASES, head(MARKET_PHASES, n=1))) {
    inner <- df %>%
      # Remove phases before starting phase
      filter(phase >= frm) %>%
      # Valuation is subject specific
