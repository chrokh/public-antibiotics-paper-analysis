...
comparable_years <- comparable_years %>%
  ...
  mutate(
         ...
         enpv_prv = cumsum((cashflow / ((1 + discount_rate_priv) ^ t)) * prob_to),
...
```{r, echo=FALSE}
print(finals %>%
  ggplot(aes(prizes, enpv_prv, color=src)) +
...
```{r, echo=FALSE}
print(finals %>%
  ggplot(aes(prizes, enpv_prv, color=intervention)) +
  geom_smooth(method='lm', se=FALSE) +
...
```{r, echo=FALSE}
ggplot(finals, aes(y=enpv_prv, x=-enpv_ind, color=intervention)) +
  geom_point() +
  facet_wrap(. ~ src, ncol=3) +
  geom_smooth(method='lm', se=FALSE) +
...
```{r}
finals$go <- factor(finals$enpv_prv >= 0, levels=c(TRUE, FALSE))
```
