...
for (curr_phase in c('M1')) {
  for (curr_src in unique(span$src)) {
    tmp <- span %>%
      ...
      kable(caption=sprintf('Mean (across indications) cost-savings of direct funding (per go-decision) of %s prize for %s', curr_phase, curr_src)) %>%
      print
  }
}
```
...
for (curr_phase in c('M1')) {
  for (curr_src in unique(span$src)) {
    tmp <- span %>%
      ...
      kable(caption=sprintf('Mean (across indications) cost-savings of direct funding (per market entry) of %s prize for %s', curr_phase, curr_src)) %>%
      print
  }
}
```
...
However, we only keep the `r M=300; M` first samples for each source/indication since significantly fewer samples are needed when prize size and public inefficiencies are not stochastic.
...
```{r, ljhfsk, dependson=-1, echo=FALSE}
scenarioA %>%
  group_by(src, prize_phase) %>%
  summarize(mean_diff = mean(inef_dir_enpv - ind_enpv)) %>%
