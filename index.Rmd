...
Sertkaya et. al (2014) also consider costs for a few additional supply chain activities as listed below.
The costs are spread across various phases as indicated by the fractions under the corresponding phase columns.
These figures stem from Table 9 of Sertkaya et. al (2014).
...
sertkaya2014_additional_dist <-
  data.frame(activity,
             min = c(2.4, 18.7, 10.7, 69.6),
             mid = c(2.7, 26.8, 13.4, 83),
             max = c(2.9, 34.8, 16.1, 96.3),
             PC = 0,
             P1 = c(1/3, 0.5, 0,    0),
             P2 = c(1/3, 0.5, 0,    0),
             P3 = c(1/3, 0,   0.75, 0),
...
```{r, print-sertkaya-additional-dist, echo=FALSE, dependson=-1}
kable(sertkaya2014_additional_dist, caption='sertkaya2014_additional_dist')
```
...
sertkaya2014_dev <- data.frame(
  ...
  src = dist$indication,
  phase = dist$phase,
  time = runif(N * num_phases * num_indications, dist$time_min, dist$time_max),
...
num_sertkaya2014_subjects <- length(sertkaya2014_subjects)
num_additional <- nrow(sertkaya2014_additional_dist)
dist <- sertkaya2014_additional_dist

sertkaya2014_additional <-
  data.frame(subject = rep(sertkaya2014_subjects, each = num_additional),
             cost = rtriangle2(num_additional * num_sertkaya2014_subjects,
                               dist$min, dist$max, dist$mid),
             PC = dist$PC,
             P1 = dist$P1,
             P2 = dist$P2,
             P3 = dist$P3,
...
We then merge the development sample with the supply chain activities sample into a single sample,
and add the additional supply chain costs to the sampled phase costs.

```{r, join-sertkaya-dev-additional}
sertkaya2014_dev_and_additional <-

  # Merge dev samples with additional supply chain costs samples (by left join)
  merge(sertkaya2014_dev, sertkaya2014_additional,
             by=c('subject', 'phase'), all.x = TRUE) %>%

  # Add supply chain activities cost to phase cost
  mutate(cost = cost + add_cost) %>%
...
subjects <- unique(sertkaya2014_dev$subject)
...
We then merge the development sample that's been merged with the supply chain costs sample,
with the market sample into a single Sertkaya sample.

```{r, join-sertkaya-dev-market}
sertkaya2014 <-
  rbind(sertkaya2014_dev_and_additional,
...
sertkaya2014_agents <-
  data.frame(subject = subjects,
             discount_rate_priv = runif(length(sertkaya2014_subjects), 0.09, 0.24))
