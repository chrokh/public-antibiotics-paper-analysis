...
finals %>%
  ...
  xlab('Prize (log)') + ylab('-Indirect ENPV per entry (including terminations)') +
  theme(legend.position = 'top')
```
...
finals %>%
  ...
  scale_y_continuous(trans = 'log',
                     breaks = c(1 %o% 10^(0:3)),
                     minor_breaks = c(1:9 %o% 10^(0:3))) +
  xlab('Prize (log)') + ylab('-Indirect ENPV per go-decision') +
  theme(legend.position = 'top')
```
...
for (curr in sources) {
  ...
  p1 <- sub %>%
    ggplot(aes(go_pred_prize*100, -enpv_ind_per_exit, color=intervention)) +
    geom_smooth(method='loess', se=FALSE) +
    facet_wrap(. ~ src) +
